...
public class MailService implements IMailService {
    ...
    //스프링 Value임 ,      Value가 두 개가 있음 스프링Value는 프로퍼리티에서 값을 가져오는 거고 lombok는 간소화 할 때 쓰는 거임
    ...
    public int doSendMail(MailDTO pDTO) {
        ...
        //전달 받은 DTO로부터 데이터 가져오기(DTO객체가 메모리에 올라가지 않아 null이 발생할 수 있기 때문에 에러 방지 차원에서 if문 사용함
        if (pDTO == null) {
            pDTO = new MailDTO();
